...
class ReportsManagementPage extends StatelessWidget {
  ...
  void _showBanDialog(BuildContext context, String reportedUserId, String reportId) {
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  if (selectedDuration == '1 hour') expiration = now.add(const Duration(hours: 1));
                  else if (selectedDuration == '24 hours') expiration = now.add(const Duration(hours: 24));
                  else if (selectedDuration == '7 days') expiration = now.add(const Duration(days: 7));
                  
                  await FirebaseFirestore.instance.collection('users').doc(reportedUserId).update({
                    'isBanned': true,
                    'banReason': reasonController.text.trim(),
                    'banExpiration': expiration != null ? Timestamp.fromDate(expiration) : FieldValue.delete(),
                  });
  ...
  Widget _buildReportCard(BuildContext context, DocumentSnapshot reportDoc, bool isFeedback) {
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(isFeedback ? 'Category: ${category ?? "General"}' : 'Type: $targetType', 
            ...
            if (isFeedback && location != null) ...[
              Text('Subcategory: $location', style: const TextStyle(fontSize: 13, fontStyle: FontStyle.italic)),
              const SizedBox(height: 8),
            ],
            if (!isFeedback) Text('Reported User ID:\n$reportedUserId', style: const TextStyle(fontSize: 12)),
            const SizedBox(height: 4),
            Text('From User ID: $reporterId', style: const TextStyle(fontSize: 12, color: Colors.grey)),
            const SizedBox(height: 12),
            const Text('Reason / Message:', style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            Wrap(
              ...
              children: [
                ...
                if (!isFeedback && reportedUserId.isNotEmpty && reportedUserId != 'admin')
                    OutlinedButton.icon(
              ...
              Row(
                ...
                children: [
                  TextButton(
                    ...
                    child: const Text('Reject/Close', style: TextStyle(color: Colors.red)),
                  ),
                  if (!isFeedback)
                    ElevatedButton(
                      onPressed: () {
      ...
      child: Scaffold(
        ...
        body: StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            final userReports = reports.where((doc) => (doc.data() as Map<String, dynamic>)['targetType'] != 'feedback').toList();
            final feedbacks = reports.where((doc) => (doc.data() as Map<String, dynamic>)['targetType'] == 'feedback').toList();

            Map<String, List<DocumentSnapshot>> groupedFeedbacks = {};
            for (var doc in feedbacks) {
              final cat = (doc.data() as Map<String, dynamic>)['category'] ?? 'Other';
              ...
              children: [
                userReports.isEmpty 
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) => _buildReportCard(context, userReports[index], false),
                    ),
                ...
                feedbacks.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        final category = groupedFeedbacks.keys.elementAt(index);
                        ...
                        return ExpansionTile(
                          ...
                          children: docs.map((doc) => _buildReportCard(context, doc, true)).toList(),
                        );
                      },
                    ),
              ],
