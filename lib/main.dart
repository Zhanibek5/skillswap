...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const AuthGate(),
      routes: {
        'firstPage/loadingPage.dart': (context) => const SplashScreen(),
...
class _AuthGateState extends State<AuthGate> {
  ...
  Future<void> _checkFirstLaunch() async {
    ...
    // Once we detect the very first launch, mark it as done so next run
    // skips the splash and proceeds with auth flow.
    if (first) {
  ...
  Widget build(BuildContext context) {
    if (_isFirstLaunch) {
      return const SplashScreen(); // ← Міне, осы жерде 3-5 секундтық анимация көрсетіледі
    }
